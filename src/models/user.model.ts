...
const validatedPassword = Yup.string()
  .required()
  ...
  .test(
    ...
    "Contains at least one uppercase number",
    (value) => {
      if (!value) return false;
      const regex = /^(?=.*\d)/;
      return regex.test(value);
...
const validatedConfirmPassword = Yup.string()
  .required()
...
UserSchema.post("save", async (doc, next) => {
  try {
    ...
    const contentEmail = await renderMailHtml("regristration-success.ejs", {
